...
type policyRuleConjunction struct {
	...
	npRef       *kuryrv1alpha1.NetworkPolicyReference
...
func getServiceMatchType(protocol *kuryrv1alpha1.Protocol, ipv4Enabled, ipv6Enabled bool) []*types.MatchKey {
	var matchKeys []*types.MatchKey
	switch *protocol {
	case kuryrv1alpha1.ProtocolTCP:
		if ipv4Enabled {
	...
	case kuryrv1alpha1.ProtocolUDP:
		if ipv4Enabled {
	...
	case kuryrv1alpha1.ProtocolSCTP:
		if ipv4Enabled {
...
func (c *clause) generateServicePortConjMatches(service kuryrv1alpha1.Service, priority *uint16, ipv4Enabled, ipv6Enabled bool) []*conjunctiveMatch {
...
func (c *clause) serviceToBitRanges(service kuryrv1alpha1.Service) []types.BitRange {
...
func (c *clause) addServiceFlows(client *client, ports []kuryrv1alpha1.Service, priority *uint16) []*conjMatchFlowContextChange {
	var conjMatchFlowContextChanges []*conjMatchFlowContextChange
...
func (c *client) calculateActionFlowChangesForRule(rule *types.PolicyRule) *policyRuleConjunction {
	...
	if nClause > 1 {
		...
		if rule.IsAntreaNetworkPolicyRule() && *rule.Action == kuryrv1alpha1.RuleActionDrop {
		...
		} else if rule.IsAntreaNetworkPolicyRule() && *rule.Action == kuryrv1alpha1.RuleActionReject {
			metricFlows = append(metricFlows, c.denyRuleMetricFlow(ruleOfID, isIngress))
...
func (c *policyRuleConjunction) calculateClauses(rule *types.PolicyRule, clnt *client) (uint8, binding.Table, binding.Table) {
	...
	case kuryrv1alpha1.DirectionOut:
